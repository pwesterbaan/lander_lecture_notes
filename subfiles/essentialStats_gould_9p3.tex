...
%    The graph below shows the $t$-distribution in black where the degrees of freedom are $1$, $2$, $4$, and $30$, and the Standard Normal Distribution in blue.
    ...
    %TODO: Settle on a color-blind friendly color scheme
    %TODO: https://colorbrewer2.org/?#type=diverging&scheme=RdBu&n=5

    \definecolor{projred}{HTML}{FF0000}      % Bright Red
    \definecolor{projorange}{HTML}{FF7F00}   % Orange
    \definecolor{projblue}{HTML}{00BFFF}     % Deep Sky Blue
    \definecolor{projgreen}{HTML}{008000}    % Bright Green
    \definecolor{projdarkblue}{HTML}{0000FF} % Bright Blue
    ...
    \begin{center}
      \begin{tikzpicture}[scale=1.0, declare function={
        ...
        \begin{axis}[
          % cycle list={lander_blue,
          %   blue,
          %   green!50!black,
          %   purple!80!black,
          %   red!70!orange},
          cycle list={black, projgreen, projdarkblue, projorange, projred},
          line legend,
          legend pos=north east,
          legend cell align={left},
          every axis plot post/.append style={
            domain={-3.2:3.2},samples=N,smooth},
          axis x line=middle,
          axis y line=none,
          width=\linewidth, height=1.5*\axisdefaultheight,
          ticklabel style={font=\normalsize,inner sep=0.5pt,fill=white,opacity=1.0, text opacity=1},
          ]
          \addplot+[-, line width=1pt, legend] expression[domain=-3.5:3.5]
            {gauss(x,0,1)} node [pos=0.5, above right] {$N(0,1)$};
          \addlegendentry{\ $N(0,1)$};
          %This plot needs to be compiled using
          %"pdflatex --shell-escape" in order to utilize gnuplots
          \pgfplotsinvokeforeach {25,4,2,1}{
            \def\n{#1} %explicitly define \n for gnuplots
            \addplot gnuplot [smooth, no marks, line width=1pt]{\students};
            \addlegendentry{\ $t (\textnormal{df}=\pgfmathprintnumber{#1})$};
            }
        \end{axis}
      \end{tikzpicture}
    \end{center}
    ...
    \end{ex*}
    \begin{center}
      \begin{tabular}{@{}*{5}{M{0.085\linewidth}}@{}}\toprule
        18.96&
        20.65&
        17.47&
        19.38&
