...
\begin{document}
  ...
  \newcommand{\chFive}{%
    Negative exponents: & $\mathllap{x^{-m}}=\dfrac{1}{x^m}$\quad or\quad $\parens[\bigg]{\dfrac{a}{b}}^{-m}=\parens[\bigg]{\dfrac{b}{a}}^m$\\
    ...
    Change of base formula: & $\mathllap{\log_{\textcolor{lander_blue}{a}}(\textcolor{red}{x})}=\dfrac{\log_b(\textcolor{red}{x})}{\log_b(\textcolor{lander_blue}{a})}=\dfrac{\log(\textcolor{red}{x})}{\log(\textcolor{lander_blue}{a})}=\dfrac{\ln(\textcolor{red}{x})}{\ln(\textcolor{lander_blue}{a})}$\\
    Exponent within a Logarithm: &$\mathllap{\ln(\textcolor{red}{x}^{\textcolor{greenish}{y}})}=\textcolor{greenish}{y}\ln(\textcolor{red}{x})$\\
    % &$a^{\log_a(x)}&=x& \log_a(a^x)&=x$
  }
  \newcommand{\chSix}{%
    Simple Interest: & $\mathllap{I}=Prt$\\
    Future value (simple interest):& $\mathllap{S}=P+I=P+Prt=P(1+rt)$\\
    Return on Investment:& $\mathllap{ROI}=\frac{\textnormal{Gains}}{\textnormal{Cost}}$\\
    % Future value (compound $m$ times/yr):& $\mathllap{S}=P\parens{1+\dfrac{r}{m}}^{mt}$\\
    Future value (compound $m$ times/yr):& $\mathllap{S}=P\parens{1+i}^n$ \hfill $i=\dfrac{r}{m}$,\ \ $n=mt$\\
%    Future value (compound $m$ times/yr):& $\mathllap{S}=P\parens{1+i}^n$, \hfill where $i=\dfrac{r}{m}$ and $n=mt$\\
    Future value (compound continuously):& $\mathllap{S}=Pe^{rt}$\\[5pt]
%    & $\mathllap{S_{\angln i}}=\sbrkt{\dfrac{(1+i)^n-1}{i}}$, \hfill where $i=\dfrac{r}{m}$ and $n=mt$\\
    Future value of Ordinary Annuity:& $\mathllap{S}=R\cdot \sbrkt{\dfrac{(1+i)^n-1}{i}}$\\[10pt] % \hfill $i=\dfrac{r}{m}$,\ \ $n=mt$\\[10pt]
...
%    Present value of Annuity Due:& $\mathllap{A_{(n,\textnormal{due})}}=R\cdot \sbrkt{\dfrac{1-\parens{1+i}^{-n}}{i}}(1+i)$\\
  ...
  \begin{center}
    \formulaSheetTitle{Math 121 Formula Sheet}
    \vspace*{\baselineskip}
