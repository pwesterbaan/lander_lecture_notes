...
\relscale{\relscalefact}
  ...
  \begin{defn*}[Vertical Shift]
    Given a function $f(x)$, a new function $g(x)=f(x)+k$, where $k$ is a constant, is a \textbf{vertical shift} of the function $f(x)$. All the output values shift vertically by $k$ units (\textbf{up} if $k>0$, \textbf{down} if $k<0$).
  \end{defn*}
  \begin{ex*}
  ...
  %TODO sequence of transformations
